...
import net.ninjaworks.ninjamod.recipe.BoundaryBlockRecipe;
...
public class OperatorStationBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory, ImplementedInventory {
    ...
    public OperatorStationBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.OPERATOR_STATION, pos, state);
        this.propertyDelegate = new PropertyDelegate() {
            public int get(int index) {
                switch (index) {
                    case 0: return OperatorStationBlockEntity.this.progress;
                    case 1: return OperatorStationBlockEntity.this.maxProgress;
                    default: return 0;
                }
            }
            ...
            public void set(int index, int value) {
                switch(index) {
                    case 0: OperatorStationBlockEntity.this.progress = value; break;
                    case 1: OperatorStationBlockEntity.this.maxProgress = value; break;
                }
            }
    ...
    public Text getDisplayName() {
        return Text.literal("Operator Station");
    }
    ...
    public ScreenHandler createMenu(int syncId, PlayerInventory inv, PlayerEntity player) {
        ...
        return new OperatorStationScreenHandler(syncId, inv, this, this.propertyDelegate);
    }
    ...
    protected void writeNbt(NbtCompound nbt) {
        ...
        nbt.putInt("operator_station.json.progress", progress);
        nbt.putLong("operator_station.json.energy", energyStorage.amount);
        nbt.put("operator_station.json.variant", fluidStorage.variant.toNbt());
        nbt.putLong("operator_station.json.fluid", fluidStorage.amount);
    }
    ...
    public boolean canInsert(int slot, ItemStack stack, @Nullable Direction side) {
        Direction localDir = this.getWorld().getBlockState(this.pos).get(OperatorStationBlock.FACING);
    ...
    public static void tick(World world, BlockPos blockPos, BlockState state, OperatorStationBlockEntity entity) {
        if(world.isClient()) {
            return;
        }
    ...
    private static void extractFluid(OperatorStationBlockEntity entity) {
        try(Transaction transaction = Transaction.openOuter()) {
    ...
    private static void transferFluidToFluidStorage(OperatorStationBlockEntity entity) {
        try(Transaction transaction = Transaction.openOuter()) {
    ...
    private static boolean hasFluidSourceInSlot(OperatorStationBlockEntity entity) {
        return entity.getStack(0).getItem() == ModFluids.SOAP_WATER_BUCKET;
    }

    private static boolean hasEnoughFluid(OperatorStationBlockEntity entity) {
        return entity.fluidStorage.amount >= 500; // mB amount!
    }

    private static void extractEnergy(OperatorStationBlockEntity entity) {
        try(Transaction transaction = Transaction.openOuter()) {
    ...
    private static boolean hasEnoughEnergy(OperatorStationBlockEntity entity) {
        return entity.energyStorage.amount >= 32;
    }

    private static boolean hasEnergyItem(OperatorStationBlockEntity entity) {
        return entity.getStack(0).getItem() == ModItems.ENERGY_CRYSTAL;
    }

    private static void craftItem(OperatorStationBlockEntity entity) {
        SimpleInventory inventory = new SimpleInventory(entity.size());
        for (int i = 0; i < entity.size(); i++) {
            inventory.setStack(i, entity.getStack(i));
        }

        Optional<BoundaryBlockRecipe> recipe = entity.getWorld().getRecipeManager()
                .getFirstMatch(BoundaryBlockRecipe.Type.INSTANCE, inventory, entity.getWorld());
    ...
    private static boolean hasRecipe(OperatorStationBlockEntity entity) {
        SimpleInventory inventory = new SimpleInventory(entity.size());
        for (int i = 0; i < entity.size(); i++) {
            inventory.setStack(i, entity.getStack(i));
        }

        Optional<BoundaryBlockRecipe> match = entity.getWorld().getRecipeManager()
                .getFirstMatch(BoundaryBlockRecipe.Type.INSTANCE, inventory, entity.getWorld());
