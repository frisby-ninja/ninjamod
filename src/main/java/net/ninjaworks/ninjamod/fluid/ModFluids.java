package net.ninjaworks.ninjamod.fluid;
...
public class ModFluids {
    ...
    public static void register() {
        STILL_SOAP_WATER = Registry.register(Registries.FLUID,
                new Identifier(NinjaMod.MODID, "soap_water"), new SoapWaterFluid.Still());
        FLOWING_SOAP_WATER = Registry.register(Registries.FLUID,
                new Identifier(NinjaMod.MODID, "flowing_soap_water"), new SoapWaterFluid.Flowing());

        SOAP_WATER_BLOCK = Registry.register(Registries.BLOCK, new Identifier(NinjaMod.MODID, "soap_water_block"),
                new FluidBlock(ModFluids.STILL_SOAP_WATER, FabricBlockSettings.copyOf(Blocks.WATER)){ });
        SOAP_WATER_BUCKET = Registry.register(Registries.ITEM, new Identifier(NinjaMod.MODID, "soap_water_bucket"),
                new BucketItem(ModFluids.STILL_SOAP_WATER, new FabricItemSettings().recipeRemainder(Items.BUCKET).maxCount(1)));

        ItemGroupEvents.modifyEntriesEvent(ModItemGroups.TOOLS).register(entries -> entries.add(SOAP_WATER_BUCKET));
    }
}